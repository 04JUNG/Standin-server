...
**구현 방향**: BFF는 **Python/FastAPI로 결정**(팀 Python 인프라 재사용). 스택·엔드포인트 매핑·Job 래핑·인증·모듈 구조·단계별 계획은 **`docs/BFF_DESIGN.md`** 참조.

**⚠ 확인 필요(팀)**
1. BFF를 **누가/어느 저장소에** 만드나(앱 서버 담당자·레포명, 예: `Standin-bff`).
2. BFF↔추론 서버 사이 내부 인증(서비스 토큰/네트워크 격리) 둘지 — 추론 서버는 무인증이라 공개 노출 금지.
3. Job: BFF가 동기 `/analyze`를 감싸 폴링을 제공(권장). 추론이 길어지면 추론 서버에도 Job 도입 재검토.
