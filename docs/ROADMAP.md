...
>
> ⚠ **2026-07-31 갱신**: 직전 목표는 **작가 실사용 30분 시연**이다.
> 그 범위의 컷라인·체크리스트는 `docs/MVP_RELEASE.md`가 단일 소스이며, 충돌 시 그쪽이 우선한다.
> 이 문서는 시연 이후를 포함한 전체 우선순위를 유지한다.
...
- **파인튜닝** — 러프↔정답포즈 라벨쌍 데이터가 0건. 시연에서 나온 실패 컷을 모으는 게 선행 조건.
- **LLM으로 관절 조정** — 채택 불가. VLM은 좌표를 생성하지 않는다(`CLAUDE.md` 불변식 1). 관절 조정은 refine(고전 최적화)이 담당(`REFINE_DESIGN.md` §0-1).
- 전체 관절 refine — 시연은 팔·다리 축소판만. 손목·척추·목은 러프 노이즈에 취약.
