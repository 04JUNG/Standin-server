...
핵심 설계
---------
* **QA 후보 CHAIN_TRANSPORT_V3는 팔다리를 순차 최소회전으로 푼다.**
...
# ---------------------------------------------------------------------------
_QA_VARIANT_NAME = "CHAIN_TRANSPORT_V3"
_QA_CHAIN_SOLVER = "sequential_min_rotation_transport"
_QA_ARM_CHAINS = {
...
_QA_FOOT_INCREMENT_MAX_RAD = math.radians(120.0)
    ...
    *,
    ...
    for name, bones in _QA_LEG_CHAINS.items():
        ...
        if maps is None:
            fallback_chain(name, bones, reason, diag)
            continue
        # 발은 solved shin에서 이어지는 incremental H2가 120° 이내일 때만 푼다.
        # 그보다 큰 단일 ankle 회전은 방향 정확도보다 스킨 연속성을 우선해 parent-follow.
        predicted_foot = maps[1].to_3x3() @ ed[2]
        foot_increment = math.radians(_angle_deg(predicted_foot, es[2]))
        diag["foot_incremental_min_rotation_deg"] = math.degrees(foot_increment)
        h_foot, why_foot = _min_rotation(predicted_foot, es[2])
        solve_foot = h_foot is not None and foot_increment <= _QA_FOOT_INCREMENT_MAX_RAD
        diag["foot_direction_solved"] = solve_foot
        if solve_foot:
            maps.append(h_foot @ maps[1])
            diag["foot_transport_reason"] = "incremental ankle rotation <= 120deg"
        else:
            maps.append(maps[1].copy())
            diag["foot_transport_reason"] = (
                why_foot or f"incremental ankle rotation {math.degrees(foot_increment):.2f}deg > 120deg"
            )
        for i, canon in enumerate(bones[:3]):
            d = pair_by_canon[canon][1]
            desired_rot[canon] = maps[i] @ _rot_only(_rest_world(dst_arm, d))
            _QA_SOLVER_MODE_BY_BONE[canon] = (
                "chain_transport" if i < 2 or solve_foot else "terminal_follow"
            )
        ...
        _QA_SOLVER_MODE_BY_BONE[foot] = "chain_transport" if solve_foot else "terminal_follow"
        _QA_SOLVER_MODE_BY_BONE[toe] = "terminal_follow"
        terminals = [toe] + ([] if solve_foot else [foot])
        _QA_TERMINAL_FOLLOW_BONES.extend(terminals)
        rep.terminal_follow_bones.extend(terminals)
        diag["applied_bones"] = list(bones[:3] if solve_foot else bones[:2])
