"""V3.2.2 QA-only, mesh-gated ankle flexion swing selector.
...
PARENT_ROOT = ROOT.parent / "CHAIN_TRANSPORT_V3_2_1_PALM_ROLL_QA"
POLICY_PATH = ROOT / "ankle_swing_policy.json"
_PARENT_PACKAGE = "_standin_v321_palm_parent"
_EPS = 1.0e-8
_SIDES = ("L", "R")
...
def _load_parent():
    """Load the frozen parent under a private package name.

    A private name prevents a caller's unrelated ``converter`` module from
    silently satisfying the relative imports.
    """
    if _PARENT_PACKAGE not in sys.modules:
        init_path = PARENT_ROOT / "converter" / "__init__.py"
        ...
        if spec is None or spec.loader is None:
            raise RuntimeError("cannot load frozen V3.2.1 parent package")
        package = importlib.util.module_from_spec(spec)
        sys.modules[_PARENT_PACKAGE] = package
        spec.loader.exec_module(package)
    rt = importlib.import_module(f"{_PARENT_PACKAGE}.retarget")
    bone_map = importlib.import_module(f"{_PARENT_PACKAGE}.bone_map")
    return rt, bone_map
...
def load_policy(path: str | os.PathLike[str] = POLICY_PATH) -> tuple[dict, str]:
    ...
    if policy.get("status") != "QA_ONLY_NOT_PROMOTED":
        raise RuntimeError("ankle swing policy must remain QA-only")
    ...
    parent_hash = _sha256(PARENT_ROOT / "converter" / "retarget.py")
