"""V3.2.3 QA-only stronger swing selector with actual-mesh rescue targets."""
...
PARENT_PATH = (
    ROOT.parent / "CHAIN_TRANSPORT_V3_2_2_ANKLE_SWING_QA" / "ankle_swing_safe.py"
)
POLICY_PATH = ROOT / "ankle_clearance_policy.json"
_PARENT_MODULE = "_standin_v322_ankle_swing_parent"
_SIDES = ("L", "R")
...
def _load_v322():
    if _PARENT_MODULE not in sys.modules:
        spec = importlib.util.spec_from_file_location(_PARENT_MODULE, PARENT_PATH)
        if spec is None or spec.loader is None:
            raise RuntimeError("cannot load V3.2.2 ankle swing parent")
        module = importlib.util.module_from_spec(spec)
        sys.modules[_PARENT_MODULE] = module
        spec.loader.exec_module(module)
    return sys.modules[_PARENT_MODULE]
...
def load_policy(path: str | os.PathLike[str] = POLICY_PATH) -> tuple[dict, str]:
    ...
    if policy.get("schema_version") != 1 or policy.get("status") != "QA_ONLY_NOT_PROMOTED":
        raise RuntimeError("invalid V3.2.3 QA policy")
