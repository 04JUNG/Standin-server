"""QA-only source-toe / target-plantar pitch transfer.
...
V323_PATH = (
    ROOT.parent / "CHAIN_TRANSPORT_V3_2_3_ANKLE_CLEARANCE_QA"
    / "ankle_clearance_safe.py"
)
POLICY_PATH = ROOT / "foot_plant_policy.json"
_V323_MODULE = "_standin_v323_ankle_clearance_parent"
_SIDES = ("L", "R")
_EPS = 1.0e-8
...
def _load_v323():
    if _V323_MODULE not in sys.modules:
        spec = importlib.util.spec_from_file_location(_V323_MODULE, V323_PATH)
        if spec is None or spec.loader is None:
            raise RuntimeError("cannot load V3.2.3 diagnostic predecessor")
        module = importlib.util.module_from_spec(spec)
        sys.modules[_V323_MODULE] = module
        spec.loader.exec_module(module)
    return sys.modules[_V323_MODULE]
...
def load_policy(path: str | os.PathLike[str] = POLICY_PATH) -> tuple[dict, str]:
    ...
    if policy.get("status") != "QA_ONLY_NOT_PROMOTED":
        raise RuntimeError("V3.2.4 must remain QA-only")
