...
* **QA 후보 CHAIN_TRANSPORT_V3_2_PELVIS_BOUNDARY는 Hips-UpLeg 경계를 잇는다.**
  ...
  이는 허벅지 방향을 만족하는 회전 중 Hips 수송에 가장 가까운 해다. 따라서 V3.1과
  같은 허벅지 방향을 유지하면서 Hips/UpLeg 공유 웨이트 경계의 미정 roll을 부모에서
  이어받는다. 한쪽이라도 이 parent-coherent seed가 퇴화하면 양쪽 활성 다리를 exact
  V3.1 수학으로 복구한다. Hips, spine, 팔, 손은 V3.1과 동일하다.

* **동결 V3.1의 발목 제한은 그대로 보존한다.**
  발목의
...
def _signed_twist_deg(rotation: Matrix, axis: Vector) -> float:
    """rotation의 axis 성분을 [-180, 180]도에서 반환한다.

    V3.1과 parent-coherent 해는 같은 target thigh edge를 같은 source direction으로
    보낸다. 두 해의 상대회전은 수치 오차를 제외하면 source thigh 축 twist이므로,
    이 값은 골반 경계에서 제거되는 미정 roll의 독립 진단값이다.
    """
    if axis.length <= _DIR_EPS:
        return 0.0
    u = axis.normalized()
...
def retarget(
    ...
    *,
    ...
    def transport_maps(es: list[Vector], ed: list[Vector],
                       initial: Matrix | None = None
                       ) -> tuple[list[Matrix] | None, str, list[float]]:
        """부모가 수송한 target edge에서 source edge로 최소회전만 누적한다."""
        q = initial.copy() if initial is not None else Matrix.Identity(4)
        maps: list[Matrix] = []
    ...
    # Hips는 계속 legacy C다. 그 결과가 target Hips rest에 가한 proper rotation만
    # 추출해 양쪽 다리의 공통 parent seed로 쓴다. source/target Hips 본 방향 자체는
    # 리그 관례이므로 swing 대상으로 사용하지 않는다.
    d_hips = pair_by_canon[_ROOT_CANON][1]
    hips_rest_rot = _rot_only(_rest_world(dst_arm, d_hips))
    hips_transport = legacy_rot[_ROOT_CANON] @ hips_rest_rot.inverted()
    pelvis_diag: dict = {
        "mode": "parent_coherent_projection",
        "parent_variant": _QA_PARENT_VARIANT,
        "parent_retarget_sha256": _QA_PARENT_RETARGET_SHA256,
        "hips_transport_rotation_deg": _rotation_error_deg(
            Matrix.Identity(4), hips_transport
        ),
        "hips_transport_determinant": hips_transport.to_3x3().determinant(),
        "hips_rotation_unchanged": True,
        "root_translation_unchanged": True,
        "bilateral_exact_v31_fallback": False,
        "active_chains": [],
        "fallback_chains": [],
    }
    ...
    def solve_leg_maps(es: list[Vector], ed: list[Vector], initial: Matrix
        ...
        """주어진 seed에서 thigh/shin과 동결 V3.1 발목을 순서대로 푼다."""
        maps, reason, increments = transport_maps(es[:2], ed[:2], initial)
        if maps is None:
            return None, reason

        predicted_foot = maps[1].to_3x3() @ ed[2]
        requested_deg = _angle_deg(predicted_foot, es[2])
        ...
        if h_foot is None:
            ...
            amount["residual_direction_deg"] = max(
                0.0, requested_deg - actual_applied
            )
        ...
        return {
            ...
            "foot_incremental_min_rotation_deg": requested_deg,
            "ankle_transport": amount,
            "full_foot": full_foot,
            "partial_foot": partial_foot,
            "apply_foot": apply_foot,
        }, ""

    # 다리: 승인된 V3.1 해와 parent-coherent 해를 모두 계산한다. 새 해는 target
    # Hips rest->output 수송에서 시작하고, 같은 source thigh 방향까지 필요한 최소 swing만
    # 추가한다. 한쪽 parent seed라도 퇴화하면 모든 활성 다리를 exact V3.1로 되돌린다.
    leg_records: list[dict] = []
    bilateral_boundary_failed = False
    for name, bones in _QA_LEG_CHAINS.items():
        diag = {"kind": "leg", "bones": list(bones), "fallback": False}
        _QA_CHAIN_FRAME_DIAGNOSTICS[name] = diag
        if not all(c in pair_by_canon for c in bones):
            fallback_chain(name, bones, "canonical chain 매핑 누락", diag)
            bilateral_boundary_failed = True
            pelvis_diag["fallback_chains"].append(name)
            continue
        src_rest_pts = [_source_rest_point(src_arm, src_tbl, c, mirror) for c in bones]
        dst_rest_pts = [_target_rest_point(dst_arm, dst_tbl, c) for c in bones]
        ers, why_ers, _ = _edge_set(src_rest_pts)
        ...
        v31, v31_reason = solve_leg_maps(es, ed, Matrix.Identity(4))
        if v31 is None:
            fallback_chain(name, bones, v31_reason, diag)
            bilateral_boundary_failed = True
            pelvis_diag["fallback_chains"].append(name)
            continue

        parent, parent_reason = solve_leg_maps(es, ed, hips_transport)
        if parent is None:
            bilateral_boundary_failed = True
            _QA_PELVIS_BOUNDARY_FALLBACKS.append(name)
            pelvis_diag["fallback_chains"].append(name)
            diag.update({
                "parent_coherent_available": False,
                "parent_coherent_failure_reason": parent_reason,
            })
        else:
            correction = parent["maps"][0] @ v31["maps"][0].inverted()
            correction_deg = _rotation_error_deg(Matrix.Identity(4), correction)
            correction_twist = _signed_twist_deg(correction, es[0])
            diag.update({
                "parent_coherent_available": True,
                "parent_predicted_thigh_mismatch_deg": parent["increments"][0],
                "v31_incremental_min_rotation_deg": v31["increments"],
                "parent_coherent_incremental_min_rotation_deg": parent["increments"],
                "v31_boundary_correction_deg": correction_deg,
                "v31_boundary_correction_twist_deg": correction_twist,
                "v31_boundary_correction_non_twist_residual_deg": abs(
                    correction_deg - abs(correction_twist)
                ),
                "parent_coherent_thigh_direction_error_deg": _angle_deg(
                    parent["maps"][0].to_3x3() @ ed[0], es[0]
                ),
                "v31_ankle_transport": v31["ankle_transport"],
                "parent_coherent_ankle_transport": parent["ankle_transport"],
            })
        pelvis_diag["active_chains"].append(name)
        leg_records.append({
            "name": name,
            "bones": bones,
            "diag": diag,
    ...
    if bilateral_boundary_failed:
        pelvis_diag["bilateral_exact_v31_fallback"] = True
        for record in leg_records:
            if record["name"] not in _QA_PELVIS_BOUNDARY_FALLBACKS:
                _QA_PELVIS_BOUNDARY_FALLBACKS.append(record["name"])
        if leg_records:
            rep.warnings.append(
                "V3.2 pelvis boundary seed 퇴화 -> 양쪽 활성 다리 exact V3.1 복구"
            )
    ...
    for record in leg_records:
        ...
        bones = record["bones"]
        diag = record["diag"]
        use_parent = not bilateral_boundary_failed and record["parent"] is not None
        selected = record["parent"] if use_parent else record["v31"]
        maps = selected["maps"]
        full_foot = selected["full_foot"]
        partial_foot = selected["partial_foot"]
        apply_foot = selected["apply_foot"]
        amount = selected["ankle_transport"]
        requested_deg = selected["foot_incremental_min_rotation_deg"]

        diag.update({
            "parent_coherent_applied": use_parent,
            "bilateral_exact_v31_fallback": bilateral_boundary_failed,
            "incremental_min_rotation_deg": selected["increments"],
            "foot_incremental_min_rotation_deg": requested_deg,
            "ankle_transport": amount,
            "foot_direction_solved": full_foot,
            "foot_direction_partially_solved": partial_foot,
            "foot_transport_reason": amount["reason"],
            "post_boundary_residual_to_parent_coherent_deg": (
                0.0 if use_parent else
                None if record["parent"] is None else
                _rotation_error_deg(maps[0], record["parent"]["maps"][0])
            ),
        })
        if partial_foot:
            rep.warnings.append(
                f"{name}: V3.1 발목 soft-cap {requested_deg:.2f}° -> "
                f"{amount['applied_deg']:.2f}° (mu={amount['selected_mu']:.4f}, profile={sp})"
            )

        active_mode = (
            "chain_transport_parent_coherent" if use_parent else "chain_transport"
        )
        partial_mode = (
            "chain_transport_parent_coherent_partial" if use_parent
            else "chain_transport_partial"
        )
        for i, canon in enumerate(bones[:3]):
            d = pair_by_canon[canon][1]
            desired_rot[canon] = maps[i] @ _rot_only(_rest_world(dst_arm, d))
            _QA_SOLVER_MODE_BY_BONE[canon] = (
                active_mode if i < 2 else
                active_mode if full_foot else
                partial_mode if partial_foot else
                "terminal_follow"
            )
