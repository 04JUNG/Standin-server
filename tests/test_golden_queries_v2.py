...
# 모듈 로드 시점에 읽으면 CI 체크아웃에서 수집 단계부터 죽어 전체 실행이 중단되므로,
# 산출물이 없으면 이 모듈만 건너뛴다. 라이브러리를 받은 환경에서는 그대로 검증한다.
if not V2_PATH.exists():
    raise SkipTest(f"semantic golden query set unavailable: {V2_PATH}")
